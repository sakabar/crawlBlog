...
awk -v sum=$verb_sum '{print $1/sum"\t"$2}' $d/counted_filtered_verbs.txt | LC_ALL=C sort -nr > $d/filtered_verbs_probability.txt
  
bigram_sum=`cat $d/verb_bigram/*.txt | wc -l`
awk -v sum=$bigram_sum '{print $1/sum"\t"$2"\t"$3}' $d/counted_filtered_bigrams.txt | LC_ALL=C sort -nr > $d/filtered_bigrams_probability.txt
